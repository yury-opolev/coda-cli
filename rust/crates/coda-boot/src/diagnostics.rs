...
//! `coda-engine` entrypoint (interactive, run, serve, and the standalone
//! `coda-tui`/`coda-engine` binaries). Lives here — a core-only crate — so
//! `coda serve`'s bootstrap and `coda-engine` share it directly rather than
//! either one reaching into the TUI frontend for something TUI-independent.
//! `coda_tui::diagnostics` re-exports these so no existing TUI call site
//! needed to change when this moved out of `coda-tui`.
...
mod tests {
    ...
    /// `coda-engine` runs headless with `ProcessRole::Serve`, not `Tui` — this
    /// guards that `init` itself has no TUI-specific assumption baked in.
    #[test]
    fn init_works_for_the_serve_process_role_used_by_coda_engine() {
        let dir = tempfile::tempdir().unwrap();
        let (ctx, forward_dir) = init_in_directory(ProcessRole::Serve, None, None, "warn", dir.path().into()).expect("init succeeds");
        assert_eq!(forward_dir, dir.path());
        assert!(ctx.logger().status().path.unwrap().starts_with(dir.path()));
    }
}
