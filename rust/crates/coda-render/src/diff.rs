...
pub fn parse(text: &str) -> Diff {
    ...
    for raw in text.lines() {
        ...
        if let Some(rest) = line.strip_prefix("diff --git ") {
            ...
            in_hunk = false;
            let path = git_header_path(rest).unwrap_or_else(|| "(unknown)".to_string());
        ...
        if let Some(rest) = line.strip_prefix("+++ ") {
            ...
            if let Some(file) = current.as_mut() {
                if path == "/dev/null" {
                    ...
                    // A `diff --git` header already gave the authoritative
                    // path; only fill in when we have nothing better.
...
fn render_file(
    ...
    let header = match file.kind {
        ChangeKind::Rename => {
            ...
            format!("{}({} → {})", file.kind.label(), from, file.path)
        }
        _ => format!("{}({})", file.kind.label(), file.path),
    };
    ...
    for diff_line in &file.lines {
        ...
        for (i, chunk) in text::wrap_preformatted(&diff_line.text, content_width)
            .into_iter()
            .enumerate()
        {
