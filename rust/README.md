...
### Parity with the C# engine (retired)

The Rust engine replaced the C# one. The differential harness that proved them
equivalent — the `coda-diff` crate and its opt-in `CODA_CSHARP_ENGINE` parity
test — has been removed along with the C# tree, because a comparison with a
reference that no longer exists cannot be run or trusted.

What it established before retirement, and what still holds:

- **Zero undeclared divergences** on the shared protocol across handshake,
  history, models, listings, errors, goals, effort and schedules. Live model
  turns were always out of scope: a provider's output is not reproducible.
- **Rust is a superset.** `session/models` rows carry `reasoningLevels`,
  `inputCost`, `outputCost` and `effort`; `model/reasoningCapability` adds
  `current`/`indeterminate`/`model`/`providerId`; `session/setEffort` reports the
  effective `current` level; `session/setSystemPrompt` is Rust-only.
- **One directional difference, by design.** Applying a valid effort level to a
  model whose reasoning capability is *indeterminate* was refused by the C#
  engine and is accepted optimistically by the Rust one.

Contract tests (`coda-tui/tests/engine_contract.rs`) remain and run against
whatever `CODA_ENGINE` names, defaulting to the `coda` on `PATH`; they skip when
no engine is installed. `coda/tests/engine_parity.rs` still compares the two
*Rust* artifacts — `coda serve` against the standalone `coda-engine` — so the
unified binary and the core-only one cannot drift apart.

To read or build the retired C# implementation, check out the `csharp-final`
tag.
