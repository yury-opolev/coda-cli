...
/// <summary>
/// Sends a steering message to a running steerable agent task the caller owns — a subagent or a
/// scheduled root. The main agent may steer any such task; a managed task may steer only its own
/// running descendants. Shell tasks cannot be steered, and an unauthorized target is reported
...
public sealed class TaskSendTool : ITool
{
    ...
    public string Description =>
        "Send a steering message to a running agent task — a subagent or a scheduled task (started with task_start). Shell tasks cannot be steered.";

    public string InputSchemaJson => """
        {"type":"object","properties":{"task_id":{"type":"string","description":"The agent task id (subagent or scheduled)"},"message":{"type":"string","description":"The steering message to deliver"}},"required":["task_id","message"]}
        """;
