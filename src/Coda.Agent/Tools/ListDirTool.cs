...
public sealed class ListDirTool : ITool
{
    ...
    public Task<ToolResult> ExecuteAsync(JsonElement input, ToolContext context, CancellationToken cancellationToken = default)
    {
        ...
        else if (!ToolInput.TryResolveWithinRoot(context.WorkingDirectory, path, out dir!, out var pathError, context.AllowOutsideWorkingDirectory, context.GrantedDirectories))
        {
