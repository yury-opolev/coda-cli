...
public sealed class NotebookEditTool : ITool
{
    ...
    public async Task<ToolResult> ExecuteAsync(JsonElement input, ToolContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (!ToolInput.TryResolveWithinRoot(context.WorkingDirectory, notebookPath, out var fullPath, out var pathError, context.AllowOutsideWorkingDirectory, context.GrantedDirectories))
        {
