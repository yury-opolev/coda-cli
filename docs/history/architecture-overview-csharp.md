> # ARCHIVED — this describes the retired C# implementation
>
> This document is a snapshot of the C# engine's architecture as it stood when
> that implementation was retired. It is kept because it is a good description
> of *why* things were shaped the way they were, and several of its
> observations carried into the Rust design. **It does not describe the code
> that ships.** Every project, type and file it names (`CodaSession`,
> `LlmClient`, `Coda.Tui`, `Coda.Agent`, `LlmAuth*`) has been deleted; the
> source is reachable at the `csharp-final` tag.
>
> For the architecture of the engine that actually runs, see
> [`rust/README.md`](../../rust/README.md) — in particular its *Approach*,
> *Crates*, *Design notes* and *The two seams* sections — and
> [`docs/serve-protocol.md`](../serve-protocol.md) for the wire contract.
