...
- `src\Coda.Tui\Ui\Rendering\TerminalTextSanitizer.cs` — control/ANSI sanitizer + Spectre escape (Task 4).
...
## Task 4: TerminalTextSanitizer + TaskLogTailReader

Pure sanitizer and a bounded, non-blocking, race-tolerant UTF-8 tail reader. No Terminal.Gui. The tail reader lives in `Coda.Tui.Ui.Tasks`; the sanitizer lives in the neutral `Coda.Tui.Ui.Rendering` namespace (the plain/Spectre renderers depend on it). Both are fully headless-testable.

**Files:**
- Create: `src\Coda.Tui\Ui\Rendering\TerminalTextSanitizer.cs`
- Create: `src\Coda.Tui\Ui\Tasks\TaskLogTailReader.cs`
- Test: `tests\Coda.Tui.Tests\TerminalTextSanitizerTests.cs`
- Test: `tests\Coda.Tui.Tests\TaskLogTailReaderTests.cs`

### 4a. TerminalTextSanitizer

- [ ] **Step 1: Write the failing test** — create `tests\Coda.Tui.Tests\TerminalTextSanitizerTests.cs`
...
Run: `dotnet test .\tests\Coda.Tui.Tests\Coda.Tui.Tests.csproj --filter "FullyQualifiedName~TerminalTextSanitizerTests"`
Expected: FAIL to compile — `TerminalTextSanitizer` does not exist.

- [ ] **Step 3: Write minimal implementation** — create `src\Coda.Tui\Ui\Rendering\TerminalTextSanitizer.cs`
...
/// (keeping tab and newline, dropping carriage returns) and Unicode bidi overrides/isolates.
/// <see cref="StripAnsiEscapes"/> also drops residual bare ESCs so removal can never reform a live
/// sequence. <see cref="SanitizeForMarkup"/> additionally escapes Spectre.Console markup so bracketed
/// text from a task can never be interpreted as markup. Lives in the neutral rendering namespace
/// because the plain/Spectre renderers depend on the shared escape-stripping primitive.
/// </summary>
...
```bash
git add src/Coda.Tui/Ui/Rendering/TerminalTextSanitizer.cs tests/Coda.Tui.Tests/TerminalTextSanitizerTests.cs
git commit -m "feat(tui): add TerminalTextSanitizer (ANSI/control strip + Spectre escape)"
```
...
internal sealed class TaskBrowserOverlay : View
{
    ...
    private void RenderDetail(StringBuilder sb, TaskBrowserState state)
    {
        ...
        sb.AppendLine(TerminalTextSanitizer.Sanitize(row.Task.Description));
...
public sealed class TasksCommand : ISlashCommand
{
    ...
    // dynamic field is only stripped of control/ANSI (TerminalTextSanitizer.Sanitize) before being colored.
    internal static IReadOnlyList<string> RenderLines(IReadOnlyList<TaskSnapshot> tasks)
    {
        ...
        foreach (var t in tasks)
        {
            ...
            var desc = TerminalTextSanitizer.Sanitize(t.Description);
