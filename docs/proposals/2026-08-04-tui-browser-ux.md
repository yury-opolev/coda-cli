...
| Shell calls `SetFocus()` on the overlay on every controller change (`TerminalGuiShellBase.cs:1807-1830`) | Focus is restored to the overlay **only on show**, and thereafter to the *currently focused child* — or not at all when the overlay already contains focus. Re-focusing on every state change is a bug once children exist, not merely an inconvenience. |
